...
const THEMES: Record<
  Theme,
  { section: string; eyebrow: string; heading: string; arrow: string; card: string; chip: string }
> = {
  light: {
    section: "bg-paper-mesh",
    eyebrow: "text-navy-700/70",
    heading: "text-navy-900",
    arrow:
      "border-navy-900/15 bg-paper text-navy-900 hover:bg-navy-900 hover:text-paper disabled:hover:bg-paper disabled:hover:text-navy-900",
    card: "border-navy-900/8 bg-cream",
    chip: "text-paper/90 bg-navy-950/30",
  },
  dark: {
    section: "bg-navy-900 text-paper",
    eyebrow: "text-leaf-300/80",
    heading: "text-paper",
    arrow:
      "border-paper/25 bg-paper/5 text-paper hover:bg-paper hover:text-navy-900 disabled:hover:bg-paper/5 disabled:hover:text-paper",
    card: "border-paper/10 bg-navy-950",
    chip: "text-navy-900/90 bg-paper/80",
  },
};
...
export function PhotoGallery({
  ...
  title: ReactNode;
  photos: GalleryPhoto[];
  /** Used in alt/aria text, e.g. "מסמינר הפתיחה של תוכנית גביש". */
  subject: string;
  theme?: Theme;
  id?: string;
}) {
  ...
  return (
    <section ref={sectionRef} id={id} className={`relative overflow-hidden ${t.section}`}>
      {theme === "dark" && (
        <>
          <div aria-hidden className="absolute inset-0 bg-crystal opacity-60" />
          <div aria-hidden className="absolute inset-y-0 left-0 w-1/3 bg-stripes-soft opacity-25 pointer-events-none [mask-image:linear-gradient(90deg,black,transparent)]" />
      ...
      <div className="relative pt-24 lg:pt-32 px-6 lg:px-10 mx-auto max-w-[1400px]">
        <p className={`font-body text-sm tracking-[0.18em] uppercase font-semibold mb-4 ${t.eyebrow}`}>
          {eyebrow}
        </p>
        <div className="flex items-end justify-between gap-6 mb-10 lg:mb-14">
          <h2 className={`font-display text-display font-black leading-[0.95] ${t.heading}`}>
            {title}
          </h2>
          <div className="hidden md:flex items-center gap-3 shrink-0">
            <button
              ...
              disabled={!canRight}
              aria-label="הקודם"
              className={`w-12 h-12 rounded-full border flex items-center justify-center transition-all disabled:opacity-30 disabled:cursor-not-allowed ${t.arrow}`}
            >
      ...
      <div
        ref={trackRef}
        className="relative flex gap-5 lg:gap-7 px-6 lg:px-10 pb-24 lg:pb-32 overflow-x-auto overflow-y-hidden snap-x snap-mandatory scroll-smooth cursor-grab select-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
      ...
      >
        {photos.map((photo, i) => (
          <button
            ...
            aria-label={`הגדלת תמונה ${i + 1} מתוך ${photos.length} ${subject}`}
            className={`group relative shrink-0 snap-start h-[300px] sm:h-[380px] lg:h-[460px] rounded-3xl overflow-hidden border shadow-[0_20px_60px_-30px_rgba(15,30,71,0.5)] cursor-zoom-in focus-visible:outline-none ${t.card}`}
          >
            <img
              src={photo.thumb}
              width={photo.width}
              height={photo.height}
              alt={`רגע ${subject} - תמונה ${i + 1}`}
              loading={i < 3 ? "eager" : "lazy"}
              decoding="async"
              draggable={false}
              className="h-full w-auto max-w-none block object-cover transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.06]"
            />
            <div
              aria-hidden
            ...
            <span
              aria-hidden
              className={`absolute top-4 right-4 font-display font-black text-sm tabular backdrop-blur-sm rounded-full w-9 h-9 flex items-center justify-center ${t.chip}`}
            >
              {photo.id}
            </span>
          </button>
      ...
      <Lightbox photos={photos} subject={subject} index={openIndex} onClose={() => setOpenIndex(null)} onIndexChange={setOpenIndex} />
    </section>
  );
}
...
function Lightbox({
  ...
  return createPortal(
    <div
      role="dialog"
      aria-modal="true"
      aria-label={`תמונה ${index + 1} מתוך ${total} ${subject}`}
      data-lenis-prevent
      onClick={onClose}
      onTouchStart={onTouchStart}
      onTouchEnd={onTouchEnd}
      className="fixed inset-0 z-[1000] flex items-center justify-center bg-navy-950/92 backdrop-blur-md animate-[galleryFade_0.25s_ease-out] p-4 sm:p-8"
    >
      <button
        ref={closeRef}
        type="button"
      ...
      <style>{`@keyframes galleryFade{from{opacity:0}to{opacity:1}}`}</style>
    </div>,
    document.body
  );
}
